...
A.application_date,
P.purchase_date 
FROM visits V LEFT JOIN fitness_tests F
...
test_groups = lambda x: 'A' if pd.notnull(x) else 'B'
df['ab_test_group'] = df.fitness_test_date.apply(test_groups)
df.head()
...
application = lambda x: 'Application' if pd.notnull(x) else 'No Application'
df['is_application'] = df.application_date.apply(application)
df.head()
...
plt.title('Percent of Applicants who Purchase with Application')
plt.show()
